...
from tensorflow.keras.layers import Conv1D,  Flatten, Dense, Conv1DTranspose, Reshape, Input, LSTM
...
class VariationalAutoencoderLSTM(BaseVariationalAutoencoder):
    ...
    def _get_encoder(self):
        ...
        for i, num_filters in enumerate(self.hidden_layer_sizes):
            x = LSTM(units=128,dropout=.1,return_sequences=True,name=f'enc_lstm_{i}')(x)
            # x = Conv1D(
            #         filters = num_filters, 
            #         kernel_size=3, 
            #         strides=2, 
    ...
    def _get_decoder(self):
        ...
        for i, num_filters in enumerate(reversed(self.hidden_layer_sizes[:-1])):
            x = LSTM(units=128,dropout=.1,return_sequences=True,name=f'dec_lstm_{i}')(x)
            # x = Conv1DTranspose(
            #     filters = num_filters, 
            #         kernel_size=3, 
            #         strides=2, 
        ...
        # last de-convolution
        x = LSTM(units=128,dropout=.1,return_sequences=True,name=f'dec_lstm_{i+1}')(x)
        # x = Conv1DTranspose(
...
if __name__ == '__main__':
    ...
    vae = VariationalAutoencoderLSTM(
        seq_len=T,
        feat_dim=D,
        latent_dim = 2,
